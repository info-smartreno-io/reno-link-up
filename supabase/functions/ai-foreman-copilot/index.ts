...
serve(async (req) => {
  ...
  try {
    ...
    const systemPrompt = `You are an AI assistant for a construction Foreman. Your role is to help with:
- Daily task management and prioritization
- Crew coordination and scheduling
- Material requests and tracking
- End-of-day report guidance
- Weather impact on work
- Safety compliance reminders
- Communication with the Project Manager

Be concise, practical, and action-oriented. Provide specific recommendations when possible.
Current context: ${contextData ? JSON.stringify(contextData) : 'No additional context provided'}`;
    ...
    console.log("Foreman copilot response generated for user:", userId);
    ...
    console.error("Foreman copilot error:", error);
