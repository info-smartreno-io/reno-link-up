...
interface ArchitectBidSubmissionDialogProps {
  open: boolean;
...
export default function ArchitectBidSubmissionDialog({
  open,
  onOpenChange,
  opportunity,
  architectId,
}: ArchitectBidSubmissionDialogProps) {
    ...
    try {
      ...
      if (files.length > 0) {
        for (const file of files) {
          ...
          const fileName = `${opportunity.id}/${architectId}/${Date.now()}-${Math.random().toString(36).substring(7)}.${fileExt}`;
      ...
      const { error: insertError } = await supabase
        ...
        .insert({
          ...
          bidder_id: architectId,
          bidder_type: 'architect',
