...
export default function DesignProfessionalProfile() {
  ...
  const credentialFields = getCredentialFieldsForSpecialties(form.specialties || []);
    ...
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div><Label>Primary Service Area</Label><Input value={form.primary_service_area || ""} onChange={(e) => update("primary_service_area", e.target.value)} /></div>
      ...
      <Card>
        ...
        <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <Input
              value={(form.zip_codes_served || []).join(", ")}
              onChange={(e) => update("zip_codes_served", e.target.value.split(",").map((s: string) => s.trim()).filter(Boolean))}
            />
          </div>
          <div>
            <Label>Counties Served (comma-separated)</Label>
            <Input
              value={(form.counties_served || []).join(", ")}
              onChange={(e) => update("counties_served", e.target.value.split(",").map((s: string) => s.trim()).filter(Boolean))}
            />
          </div>
          <div>
            <Label>Travel Radius (miles)</Label>
            <Input type="number" value={form.travel_radius_miles || ""} onChange={(e) => update("travel_radius_miles", parseInt(e.target.value) || null)} />
          </div>
          <div>
